...
namespace DYN
{
    ...
    if (fErr.size() > 0)
    {
      ...
      for (map<double, int>::const_iterator it = fErr.begin(); it != fErr.end() && i < nbErr; ++it, ++i)
      {
        Trace::debug() << DYNLog(SolveParametersFError, tolerance, it->second, it->first,
                                 getFequationByLocalIndex(it->second))
                       << Trace::endline;
      }
    }
#endif
  ...
  ModelManager::setInitialCalculatedParameters()
  {
    ...
    assert(parametersInitial.size() == static_cast<size_t>(modelData()->nParametersReal + modelData()->nParametersBoolean + modelData()->nParametersInteger + modelData()->nParametersString));
    for (unsigned int i = 0; i < modelData()->nParametersReal; ++i)
    {
